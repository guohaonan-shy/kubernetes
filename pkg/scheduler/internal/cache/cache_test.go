...
func TestSchedulerCache_UpdateSnapshot(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected:           []*v1.Node{nodes[1], nodes[0]},
			expectedUsedPVCSet: sets.New[string]("test-ns/test-pvc0"),
		},
		{
		...
		},
		{
			...
			expected:           []*v1.Node{nodes[1], nodes[0]},
			expectedUsedPVCSet: sets.New[string]("test-ns/test-pvc0", "test-ns/test-pvc1"),
		},
		{
		...
		},
		{
			...
			expected:           []*v1.Node{nodes[0]},
			expectedUsedPVCSet: sets.New[string]("test-ns/test-pvc2"),
		},
		{
		...
		},
		{
			...
			expected:           []*v1.Node{nodes[1], nodes[0]},
			expectedUsedPVCSet: sets.New[string]("test-ns/test-pvc2", "test-ns/test-pvc3"),
		},
		{
			...
			expected:           []*v1.Node{nodes[0], nodes[1]},
			expectedUsedPVCSet: sets.New[string]("test-ns/test-pvc1", "test-ns/test-pvc2"),
		},
	}
